...
llm = ChatOpenAI(
    model="gpt-4o-mini", temperature=0.0, streaming=True, api_key=OPENAI_API_KEY
).configurable_fields(
...
async def serpapi(query: str) -> list[Article]:
    ...
    return [
        Article.from_serpapi_result(result) for result in results["organic_results"]
    ]
...
class QueueCallbackHandler(AsyncCallbackHandler):
    ...
    async def on_llm_new_token(self, *args, **kwargs) -> None:
        ...
        if chunk and chunk.message.additional_kwargs.get("tool_calls"):
            if (
                chunk.message.additional_kwargs["tool_calls"][0]["function"]["name"]
                == "final_answer"
            ):
...
async def execute_tool(tool_call: AIMessage) -> ToolMessage:
    ...
    return ToolMessage(
        content=f"{tool_out}", tool_call_id=tool_call.tool_calls[0]["id"]
    )
...
class CustomAgentExecutor:
    ...
    async def invoke(
        ...
        while count < self.max_iterations:
            ...
            for tool_call in tool_calls:
                agent_scratchpad.extend(
                    [tool_call, id2tool_obs[tool_call.tool_call_id]]
                )
        ...
        return (
            final_answer_call
            if final_answer
            else {"answer": "No answer found", "tools_used": []}
        )
